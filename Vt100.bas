...
Public Function term_process_char(CH As Byte)
    ...
    'ASCII Codes, for reference of select case below
    '                  Case 0      'Null
    ...
    '                  Case Else   'Text, ASCII codes 33-126
        ...
        Else
                ...
                Select Case CH
                    ...
                    Case 7
                        'Uncomment below to restore "bell"
                        'Beep                                '   <BEL> char
                    ...
                    Case 13                                 '   <CR> charage return
                        '   end of line
                        CurX = 0                            '   call DX and WWV subs
                        'Add terminater to TelnnetDXtext
                        TelnetDXtext = TelnetDXtext + vbCrLf
            
                        'Send off to DX cluster Spotting module
                        Call DXHeardList(TelnetDXtext)
            
                        'Send off to WWV info Spotting module
                        Call WWVinfo(TelnetDXtext)
            
                        'Sen off to RcvAnnouncement module
                        Call RcvAnnoncement(TelnetDXtext)
            
                        'Reset TelnetDXtext
                        TelnetDXtext = ""
            
                    Case 27                                 '   <ESC> escape char
                    ...
                    Case Else
                        ...
                        If (CH > 31) And (CH < 128) Then    'Only printable charaters
                            term_write CH
                            Mid$(ScrImage(CurY + 1), CurX, 1) = Chr$(CH)
                            Mid$(ScrAttr(CurY + 1), CurX, 1) = curattr
                            'If (CH > 31) And (CH < 128) Then    'Only printable charaters
                            'Write charater into text buffer
                            TelnetDXtext = TelnetDXtext + Chr$(CH)
                            infotext = UCase$(TelnetDXtext)
                                
                            'If Login = True And TelnetLogon <> "" And infotext = "LOGIN:" Then

                                If Login = True And TelnetLogon <> "" Then
                                    Call modLogin.Autologon("LOGIN:")
                                End If
                
                            'If Passwrd = True And TelnetPswrd <> "" And infotext = "PASSWORD" Then

                                If Passwrd = True And TelnetPswrd <> "" Then
                                    Call modLogin.Autologon("PASSWORD:")
                                End If
